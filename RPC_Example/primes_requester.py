...
def requester_main(domain_id, n, primes_per_reply):
    ...
    while in_progress:
        ...
        replies = requester.take_replies(related_request_id=request_id)

        for reply, info in replies:
            print(list(reply.primes))
            if first_reply_guid is None:
                first_reply_guid = info.source_guid
            print(f"Response coming from replier_w_guid: {info.source_guid}")
    ...
    # second request to the same replier
    prime_number_second_request = Primes.PrimeNumberRequest(
        n=n, primes_per_reply=primes_per_reply, replier_w_guid=str(first_reply_guid)
    )
    
    print(
        f"Sending a request AGAIN to calculate the prime numbers <= {n} in sequences of {primes_per_reply} or fewer elements"
    )

    request_id = requester.send_request(prime_number_second_request)
