...
async def main() -> int:
    ...
    pipeline_script = os.path.join(REPO_ROOT, "run_scripts", "active", "run_interface_agent_service_test.sh")
    env = os.environ.copy()
    env["PYTHONPATH"] = f"{REPO_ROOT}:{env.get('PYTHONPATH', '')}"
    proc = subprocess.Popen(["bash", pipeline_script], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, env=env, cwd=os.path.join(REPO_ROOT, "run_scripts", "active"))
