...
def start_calculator_service() -> subprocess.Popen:
    script = os.path.join(REPO_ROOT, "test_functions", "calculator_service.py")
    return subprocess.Popen([sys.executable, script], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
...
async def main() -> int:
    ...
    svc = start_calculator_service()
    try:
        # Wait for service node + function nodes
        ok_nodes = await wait_for(lambda: len(obs.service_nodes) >= 1 and len(obs.function_nodes) >= 4, timeout=20)
        if not ok_nodes:
