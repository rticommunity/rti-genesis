...
run_with_timeout() {
  ...
  if [[ "$script_path" == *.py ]]; then
    PYTHONPATH=$PYTHONPATH:$PROJECT_ROOT timeout "$timeout" python "$script_path" > "$log_file" 2>&1 || return $?
  else
    PYTHONPATH=$PYTHONPATH:$PROJECT_ROOT timeout "$timeout" bash "$script_path" > "$log_file" 2>&1 || return $?
  fi
}
...
main() {
  ...
  echo "For full coverage, run ./run_scripts/run_all_tests.sh"
  echo "Logs: $LOG_DIR"
  echo "=================================================="
}
