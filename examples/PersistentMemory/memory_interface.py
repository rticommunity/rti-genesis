...
"""
PersistentMemoryInterface — Interactive CLI for the PersistentMemoryAgent.

Discovers available PersistentMemoryAgents on the Genesis DDS network,
connects to one, and provides an interactive chat loop. Conversation
history persists in the agent's database across restarts.
...
logger = logging.getLogger("PersistentMemoryInterface")

INTERFACE_NAME = "MemoryCLI"
...
async def main(verbose: bool = False):
    ...
    interface = MonitoredInterface(
        ...
        service_name="MemoryInterfaceService",
    )
    ...
    try:
        # --- Agent discovery ---
        logger.info("Waiting for memory agent(s) to become available (up to 30s)...")
        try:
        ...
        if not connection_ok:
            logger.error(f"Failed to connect to '{connected_agent_name}'. Exiting.")
            await interface.close()
            return

        logger.info(f"Connected to '{connected_agent_name}'.")
        print(
            f"\nConnected to: {connected_agent_name}\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PersistentMemory Interface CLI")
    parser.add_argument(
        "-v", "--verbose",
        action="store_true",
