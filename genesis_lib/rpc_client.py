"""
Genesis RPC Client - Unified Topics with Intelligent Request Routing

This module provides the client-side implementation for Genesis RPC communication,
...
=================================================================================================
ARCHITECTURE OVERVIEW - Understanding RPC Client Design
=================================================================================================

The Genesis RPC Client implements a sophisticated request/reply pattern that provides:
...
The RPC client uses unified topics that enable:
...
The client implements a robust request/reply pattern with:
...
The client provides comprehensive service discovery capabilities:
...
This enables:
- **Dynamic Scaling**: Services can be added/removed without client changes
...
1. **Basic Function Calls**:
   ```python
   client = GenesisRPCClient("MyService")
   result = await client.call("calculate_sum", {"a": 5, "b": 3})
   ```

2. **Targeted Service Calls**:
   ```python
   client = GenesisRPCClient("MyService")
   result = await client.call("process_data", params, target_service_guid="specific-guid")
   ```

3. **Legacy API Compatibility**:
   ```python
   client = GenesisRPCClient("MyService")
   result = await client.call_function("calculate_sum", a=5, b=3)
   ```
...
# =============================================================================
# GENESIS RPC CLIENT CLASS - UNIFIED TOPICS WITH INTELLIGENT ROUTING
# =============================================================================
# The GenesisRPCClient class provides client-side RPC communication with
# unified topics, intelligent request routing, and robust error handling.
# =============================================================================
...
class GenesisRPCClient:
    def __init__(self, service_name: str = "GenesisRPCService", service_type: Optional[str] = None, participant: Optional[dds.DomainParticipant] = None, timeout: Optional[dds.Duration] = None, timeout_seconds: float = 10.0):
        """
        Initialize the Genesis RPC client.
        
        The GenesisRPCClient provides client-side RPC communication with unified topics,
        ...
        Examples:
            ...
            # With custom timeout
            client = GenesisRPCClient("MyService", timeout_seconds=30.0)
            
            # With existing DDS participant
            client = GenesisRPCClient("MyService", participant=existing_participant)
            
        Note:
            The client automatically handles service discovery and will wait for
        ...
        logger.info(f"RPC client initializing for service_name={service_name_full}")
        ...
        logger.info(
            "RPC requester initialized: req_writer_guid=%s, reply_reader_guid=%s",
    ...
    # =============================================================================
    # RPC CALL METHODS - REQUEST/REPLY COMMUNICATION
    # =============================================================================
    # Methods for making RPC calls with intelligent routing and error handling
    # =============================================================================
    ...
    async def call(self, function_name: str, params: Dict[str, Any], target_service_guid: str = "", deadline_ms: int = 10000) -> Dict[str, Any]:
        ...
        This method provides the primary interface for making RPC calls with support
        ...
        Examples:
            ...
            # With custom timeout
            result = await client.call("long_running_task", params, deadline_ms=30000)
            
        Note:
            The client automatically waits for service discovery before making requests.
        ...
        logger.info("RPC matched_replier_count=%s", self.requester.matched_replier_count)
        ...
        logger.info("RPC sent request: id=%s, func=%s", request_id, function_name)
    ...
    # =============================================================================
    # Methods for managing client lifecycle and resource cleanup
    # =============================================================================
    ...
    def close(self):
        ...
        This method performs comprehensive cleanup of all RPC client resources
        including the requester and DDS participant (if owned by this client).
        It ensures proper resource cleanup to prevent memory leaks.
        
        The cleanup process includes:
        1. Requester cleanup (closes DDS writers/readers)
        2. Participant cleanup (if owned by this client)
        ...
        Examples:
            ...
            with GenesisRPCClient("MyService") as client:
                # ... use client ...
                pass  # Automatic cleanup on exit
                
        Note:
            Always call this method when done with the RPC client to
