...
LOGS_DIR=${LOGS_DIR:-"${ROOT_DIR}/tests/stress/logs"}
...
Stress test Genesis by launching multiple agents, services, and interfaces.
This is useful for testing scalability, monitoring, and graph visualization.
...
Examples:
  # Basic stress test with 10 agents, 20 services, 1 interface for 180 seconds
  $0 -a 10 -s 20 -i 1 -t 180

  # Test with custom commands and questions
  ...
  # Add weather agent and ask weather questions
  $0 -a 1 -s 1 -i 1 \\
     --extra-agent-cmd "python3 examples/MultiAgent/agents/weather_agent.py" \\
     --interface-question "What's the weather in Seattle?" \\
     --interface-question "What is 50 * 25?"
EOF
}
...
start_timer
log "All processes started. Press Ctrl-C to stop, or wait for ${TIMEOUT_SEC}s timeout."
